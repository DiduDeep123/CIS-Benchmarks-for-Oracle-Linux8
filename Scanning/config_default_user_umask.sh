...
log_message "Ensuring default user umask is configured..."
   l_output="" l_output2="" 
   file_umask_chk() 
   { 
   ...
   if [ -z "$l_output2" ]; then 
      ...
      log_message "Default user umask is correctly configured."
      echo "DEFAULT USER UMASK CONFIG:CORRECT" >> "$RESULT_FILE"
   else 
      echo -e "\n- Audit Result:\n  ** FAIL **\n - * Reasons for audit failure * :\n$l_output2" 
      [ -n "$l_output" ] && echo -e "\n- * Correctly configured * :\n$l_output\n" 
      log_message "Default user umask is incorrectly configured."
      echo "DEFAULT USER UMASK CONFIG:INCORRECT" >> "$RESULT_FILE"
   fi 
