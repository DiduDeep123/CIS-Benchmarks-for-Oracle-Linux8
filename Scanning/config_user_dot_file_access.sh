...
log_message() {
 ...
 log_message "Checking local interactive user dot files access is configured"
   ...
   if [ -z "$l_output2" ]; then # If l_output2 is empty, we pass 
      ...
      echo -e "$l_output3\n" 
      echo "LOCAL INTERACTIVE USER DOT FILE ACCESS:CONFIGURED" >> $RESULT_FILE
   else 
      echo -e "\n- Audit Result:\n  ** FAIL **\n - * Reasons for audit failure * :\n$l_output2\n" 
      echo -e "$l_output3\n" 
      [ -n "$l_output" ] && echo -e "- * Correctly configured * :\n$l_output\n" 
      echo "LOCAL INTERACTIVE USER DOT FILE ACCESS:NOT CONFIGURED" >> $RESULT_FILE
   fi 
