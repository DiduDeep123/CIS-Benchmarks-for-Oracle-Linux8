...
log_message "Checking if Bogus ICMP responses are ignored..."
 l_output="" l_output2=""
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "BOGUS ICMP RESPONSES: IGNORED" >> $RESULT_FILE
 log_message "Bogus ICMP responses are ignored."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
  echo "BOGUS ICMP RESPONSES: REQUIRES CHANGE" >> $RESULT_FILE
 log_message "Bogus ICMP responses are not ignored."
 fi
