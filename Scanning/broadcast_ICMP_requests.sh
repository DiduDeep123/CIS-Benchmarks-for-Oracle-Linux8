...
log_message "Checking if broadcast ICMP requests are ignored..."
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "BROADCAST ICMP REQUESTS:IGNORED" >> $RESULT_FILE
 log_message "Broadcast ICMP requests are ignored."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "BROADCAST ICMP REQUESTS:REQUIRES CHANGE" >> $RESULT_FILE
 log_message "Broadcast ICMP requests are not ignored."
 fi
