...
log_message() {
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n ** PASS **"
 echo "WIRELESS INTERFACES: DISABLED" >> $RESULT_FILE
 log_message "Wireless interfaces are disabled."
 if [ -z "$l_output" ]; then
 ...
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "WIRELESS INTERFACES: NOT DISABLED" >> $RESULT_FILE
 log_message "Wireless interfaces are not disabled."
 fi
