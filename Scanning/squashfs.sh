...
log_message "Checking if squashfs kernel module is not available..."
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "SQUASHFS CHECK:SUCCESS" >> $RESULT_FILE
 log_message "squashfs kernel module check: PASS."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "SQUASHFS CHECK:FAILED" >> $RESULT_FILE
 log_message "squashfs kernel module check: FAILED."
 fi
