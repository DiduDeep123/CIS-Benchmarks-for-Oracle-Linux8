...
log_message() {
 ...
 log_message "Checking permissions on bootloader config are configured... "
...
should be group-owned by group: \"root\""
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n *** PASS ***\n- * Correctly set * :\n$l_output\n"
 echo "PERMISSIONS ON BOOTLOADER CONFIG:CONFIGURED" >> $RESULT_FILE
 log_message "Permissions on bootloader config are configured."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - * Reasons for audit 
failure * :\n$l_output2\n"
 [ -n "$l_output" ] && echo -e " - * Correctly set * :\n$l_output\n"
 echo "PERMISSIONS ON BOOTLOADER CONFIG:NOT CONFIGURED" >> $RESULT_FILE
 log_message "Permissions on bootloader config are not configured."
