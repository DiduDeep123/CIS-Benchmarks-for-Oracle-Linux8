...
log_message "Checking if packet redirect sending is disabled..."
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "PACKET REDIRECT SENDING:DISABLED" >> $RESULT_FILE
 log_message "Packet redirect sending is disabled."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "PACKET REDIRECT SENDING:ENABLED" >> $RESULT_FILE
 log_message "Packet redirect sending is not disabled."
 fi
