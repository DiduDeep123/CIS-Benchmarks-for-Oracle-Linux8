...
log_message "Checking if suspicious packets are logged..."
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "SUSPICIOUS PACKET LOGGING: YES" >> $RESULT_FILE
 log_message "Suspicious packets are logged."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "SUSPICIOUS PACKET LOGGING: NO" >> $RESULT_FILE
 log_message "Suspicious packets are not logged."
 fi
