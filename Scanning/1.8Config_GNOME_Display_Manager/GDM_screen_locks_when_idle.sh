...
log_message "Check if GDM screen locks when the user is idle..."
...
\"$l_kfile\""
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "GDM SCREEN LOCK:OK" >> $RESULT_FILE
 log_message "GDM screen lock is configured or GDM is not installed."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "GDM SCREEN LOCK:NOT CONFIGURED" >> $RESULT_FILE
 log_message "GDM screen lock is not configured."
 fi
