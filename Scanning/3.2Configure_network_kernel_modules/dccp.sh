...
log_message() {
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "DCCP CHECK: SUCCESS" >> $RESULT_FILE
 log_message "dccp kernel module is not available."

 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "DCCP CHECK: FAILED" >> $RESULT_FILE
 log_message "dccp kernel module available."
