...
log_message "Checking if ASLR is enabled..."
 l_output="" l_output2=""
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "ASLR:ENABLED" >> $RESULT_FILE
 log_message "address space layout randomization(ASLR) is enabled."

 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "ASLR:NOT ENABLED" >> $RESULT_FILE
 log_message "address space layout randomization(ASLR) is not enabled."
 fi
