...
log_message "Checking if reverse path filtering is enabled..."
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "REVERSE PATH FILTERING: ENABLED" >> $RESULT_FILE
 log_message "Reverse path filtering is enabled."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "REVERSE PATH FILTERING: NOT ENABLED" >> $RESULT_FILE
 log_message "Reverse path filtering is not enabled."
 fi
