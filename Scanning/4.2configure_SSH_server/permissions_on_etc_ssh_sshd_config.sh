...
($l_mode), owner ($l_user), and group owner ($l_group) configured"
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n *** PASS ***\n- * Correctly set * :\n$l_output\n"
 echo "/etc/ssh/sshd_config_permissions:CONFIGURED" >> $RESULT_FILE
 log_message "permissions on /etc/ssh/sshd_config are configured"
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - * Reasons for audit failure * :\n$l_output2\n"
 echo "/etc/ssh/sshd_config_permissions:NOT CONFIGURED" >> $RESULT_FILE
 log_message "permissions on /etc/ssh/sshd_config are not configured"
