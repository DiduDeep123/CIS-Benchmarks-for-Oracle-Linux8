...
($l_owner), and group owner ($l_group) configured"
 ...
 if [ -z "$l_output2" ]; then
 echo -e "\n- Audit Result:\n *** PASS ***\n- * Correctly set * :\n$l_output\n"
 echo "SSH_public_host_key_file_permissions:CONFIGURED" >> $RESULT_FILE
 log_message "permissions on SSH public host key files are configured."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - * Reasons for audit failure * :\n$l_output2\n"
 [ -n "$l_output" ] && echo -e " - * Correctly set * :\n$l_output\n"
 echo "SSH_public_host_key_file_permissions:NOT CONFIGURED" >> $RESULT_FILE
 log_message "permissions on SSH public host key files are not configured."
 fi
