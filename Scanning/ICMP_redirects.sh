...
log_message "Checking if ICMP redirects are not accepted..."
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "ICMP REDIRECTS: NOT ACCEPTED" >> $RESULT_FILE
 log_message "ICMP redirects are not accepted."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "ICMP REDIRECTS: ACCEPTED" >> $RESULT_FILE
 log_message "ICMP redirects are accepted."
 fi
