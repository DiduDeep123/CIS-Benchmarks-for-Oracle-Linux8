...
log_message "Checking if ipv6 router advertisements are not accepted..."
...
\"$l_kpname\" May be set in a file that's ignored by load procedure **\n"
 ...
 echo -e "\n- Audit Result:\n ** PASS **\n$l_output\n"
 echo "IPV6 ROUTER ADVERTISEMENTS: NOT ACCEPTED" >> $RESULT_FILE
 log_message "IPV6 router advertisements are not accepted."
 else
 echo -e "\n- Audit Result:\n ** FAIL **\n - Reason(s) for audit failure:\n$l_output2\n"
 [ -n "$l_output" ] && echo -e "\n- Correctly set:\n$l_output\n"
 echo "IPV6 ROUTER ADVERTISEMENTS: ACCEPTED" >> $RESULT_FILE
 log_message "IPV6 router advertisements are accepted."
 fi
