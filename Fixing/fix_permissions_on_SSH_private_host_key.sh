...
select_no() {
 ...
 if grep -q "SSH_private_host_key_file_permissions:NOT CONFIGURED" $RESULT_FILE; then
	read -p "Do you want to configure correct pemissions on SSH private host key file ? (y/n)" answer
	if [[ answer = [Yy] ]]; then
   ...
   select_no "SSH private host key file PERMISSIONS:REQUIRES CHANGE"
