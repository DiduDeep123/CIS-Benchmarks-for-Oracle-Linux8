...
if grep -q "SCTP CHECK: FAILED"  $RESULT_FILE; then
read -p "Do you want to remove sctp module? (y/n)" answer
if [[ answer = [Yy] ]]; then
log_message "Starting remediation of sctp module..."
 ...
 log_message "sctp kernel module not available."
fi
fi
