...
apply_sysctl() {
    ...
    params=("net.ipv4.conf.all.log_martians = 1" "net.ipv4.conf.default.log_martians = 1")
	...
	if grep -q "SUSPICIOUS PACKET LOGGING: NO" $RESULT_FILE; then
	read -p "Do you want to ensure suspicious packets are logged ? (y/n)" answer
	if [[ answer = [Yy] ]]; then
    for param in "${params[@]}"; do
        key="${param%%=*}"
        value="${param#*=}"
	...
	select_no "SUSPICIOUS PACKET LOGGING:REQUIRES CHANGE"
	fi
	fi
}
